...
import {
  ...
  ChevronLeft, ChevronRight, X, UserPlus, CheckCircle,
  Clock, Briefcase, Download, Star, Shield, ShieldCheck
} from "lucide-react";

const SEED = [
  { id: 1, name: "Alice Johnson", email: "alice@swiftsupport.com", department: "Tech Support", role: "Manager", status: "Active", created: "Jan 12, 2024", tickets: 124, rating: 5 },
  { id: 2, name: "David Smith",   email: "david@swiftsupport.com", department: "Billing",      role: "Staff",   status: "Active", created: "Feb 3, 2024", tickets: 89, rating: 4 },
  { id: 3, name: "Emily Davis",   email: "emily@swiftsupport.com", department: "Sales",        role: "Staff",   status: "On Leave", created: "Mar 18, 2024", tickets: 0, rating: 3 },
  { id: 4, name: "Michael Brown", email: "michael@swiftsupport.com", department: "Tech Support", role: "Staff",   status: "Active", created: "Apr 5, 2024", tickets: 230, rating: 5 },
];
...
const RightPanel = ({ open, onClose, entry, onSave }) => {
  const blank = { name:"", email:"", phone:"", password:"", department:"Tech Support", role:"Staff", status:"Active" };
  const [form, setForm] = useState(blank);
  const [errors, setErrors] = useState({});

  useEffect(() => { 
    setForm(entry ? { name:entry.name, email:entry.email, phone:entry.phone||"", password:entry.password||"", department:entry.department, role:entry.role, status:entry.status } : blank); 
    setErrors({});
  }, [entry, open]);
    ...
    <>
      ...
      <div style={{ position:"fixed", top:0, right:0, height:"100vh", width:"400px", background:"#fff", boxShadow:"-4px 0 32px rgba(0,114,198,0.12)", zIndex:300, display:"flex", flexDirection:"column", transform:open?"translateX(0)":"translateX(100%)", transition:"transform 0.3s cubic-bezier(.4,0,.2,1)" }}>
        ...
        <div style={{ padding:"20px 24px", borderBottom:"1px solid #e2eef8", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <p style={{ margin:0, fontSize:"15px", fontWeight:700, color:"#1a3a4a" }}>{entry ? "Edit Agent Profile" : "Add Support Agent"}</p>
            <p style={{ margin:"2px 0 0", fontSize:"12px", color:"#5a7a8a" }}>{entry ? "Update employee details" : "Register a new support staff member"}</p>
          </div>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"24px" }}>
          <div style={{ marginBottom:"16px" }}>
            ...
            <input type="text" value={form.name} onChange={h("name")} placeholder="e.g. John Doe" style={{...inp, borderColor: errors.name ? "#ef4444" : "#e2eef8"}} onFocus={focus} onBlur={e => { blur(e); if(errors.name) e.target.style.borderColor="#ef4444"; }} />
            {errors.name && <span style={{ display:"block", fontSize:"11px", color:"#ef4444", marginTop:"4px", fontWeight:500 }}>{errors.name}</span>}
          </div>

          <div style={{ marginBottom:"16px" }}>
            <label style={lbl}>Email Address *</label>
            <input type="email" value={form.email} onChange={h("email")} placeholder="e.g. john@swiftsupport.com" style={{...inp, borderColor: errors.email ? "#ef4444" : "#e2eef8"}} onFocus={focus} onBlur={e => { blur(e); if(errors.email) e.target.style.borderColor="#ef4444"; }} />
            {errors.email && <span style={{ display:"block", fontSize:"11px", color:"#ef4444", marginTop:"4px", fontWeight:500 }}>{errors.email}</span>}
          </div>

          <div style={{ marginBottom:"16px" }}>
            <label style={lbl}>Phone Number *</label>
            <input type="text" maxLength={10} value={form.phone} onChange={e => {
              const val = e.target.value.replace(/[^0-9]/g, "").slice(0, 10);
              setForm(f => ({...f, phone: val}));
              if(errors.phone && val.length === 10) setErrors(errs => ({...errs, phone: ""}));
            }} placeholder="e.g. 9876543210" style={{...inp, borderColor: errors.phone ? "#ef4444" : "#e2eef8"}} onFocus={focus} onBlur={e => { blur(e); if(errors.phone) e.target.style.borderColor="#ef4444"; }} />
            {errors.phone && <span style={{ display:"block", fontSize:"11px", color:"#ef4444", marginTop:"4px", fontWeight:500 }}>{errors.phone}</span>}
          </div>

          <div style={{ marginBottom:"16px" }}>
            <label style={lbl}>Password *</label>
            <input type="password" value={form.password} onChange={h("password")} placeholder="Create a strong password" style={{...inp, borderColor: errors.password ? "#ef4444" : "#e2eef8"}} onFocus={focus} onBlur={e => { blur(e); if(errors.password) e.target.style.borderColor="#ef4444"; }} />
            {errors.password && <span style={{ display:"block", fontSize:"11px", color:"#ef4444", marginTop:"4px", fontWeight:500 }}>{errors.password}</span>}
          </div>

          <div style={{ marginBottom:"16px" }}>
            <label style={lbl}>Department</label>
            <select value={form.department} onChange={h("department")} style={{ ...inp, cursor:"pointer" }}>
          ...
          <div style={{ marginBottom:"16px" }}>
            ...
            <select value={form.role} onChange={h("role")} style={{ ...inp, cursor:"pointer" }}>
          ...
          <div style={{ background:"#f0f7ff", borderRadius:"12px", padding:"14px 16px", border:"1px solid #d0eaf9" }}>
            ...
            <p style={{ margin:0, fontSize:"11.5px", color:"#5a7a8a", lineHeight:1.7 }}>
              Agents with the <b>Manager</b> role have access to export customer data and override billing issues.
            </p>
          </div>
        </div>
    ...
    <div ref={ref} style={{ position:"relative" }}>
      ...
      {isOpen && (
        <div style={{ position:"absolute", right:0, top:"30px", background:"#fff", borderRadius:"12px", boxShadow:"0 8px 30px rgba(0,0,0,0.13)", border:"1px solid #e2eef8", width:"130px", zIndex:100, overflow:"hidden", animation:"fadeDown 0.15s ease" }}>
          <button onClick={() => { onEdit(); setOpenId(null); }} style={{ ...btnBase, color:"#1a3a4a" }}><Edit2 size={13} /> Edit Profile</button>
...
const AddAgent = () => {
  ...
  const handleSave = form => {
    ...
    if (editEntry) {
      ...
      newData = [...data, { ...form, id: Date.now(), created: new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }), tickets: 0, rating: 0 }];
    }
    setData(newData);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["Name", "Email", "Department", "Role", "Status", "Joined", "Tickets Handled", "Rating"];
    const rows = filtered.map(r => [r.name, r.email, r.department, r.role, r.status, r.created, r.tickets, r.rating]);
    ...
    <>
      ...
      <div style={{ padding:"24px", minHeight:"100%", background:"#f0f7ff", fontFamily:"'Inter',sans-serif" }}>
        ...
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:"20px", flexWrap:"wrap", gap:"12px" }}>
          <div>
            <h2 style={{ margin:0, fontSize:"20px", fontWeight:700, color:"#1a3a4a" }}>Support Agents</h2>
            <p style={{ margin:"4px 0 0", fontSize:"13px", color:"#5a7a8a" }}>Manage your customer support team and staff accounts.</p>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:"10px", flexWrap:"wrap" }}>
            {/* Search */}
            <div style={{ position:"relative" }}>
              <Search size={14} style={{ position:"absolute", left:"11px", top:"50%", transform:"translateY(-50%)", color:"#9ab0be" }} />
              <input className="agent-search" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search agents..." style={{ paddingLeft:"32px", paddingRight:"12px", height:"36px", border:"1.5px solid #e2eef8", borderRadius:"10px", fontSize:"13px", color:"#1a3a4a", background:"#fff", width:"210px", transition:"border-color 0.2s" }} />
            </div>
            {/* Filter */}
            <div ref={filterRef} style={{ position:"relative" }}>
              ...
              {filterOpen && (
                <div style={{ position:"absolute", right:0, top:"42px", background:"#fff", borderRadius:"14px", boxShadow:"0 8px 30px rgba(0,0,0,0.12)", border:"1px solid #e2eef8", padding:"16px", zIndex:100, minWidth:"200px", animation:"fadeDown 0.15s ease" }}>
                  <p style={{ margin:"0 0 8px", fontSize:"11px", fontWeight:700, color:"#9ab0be", textTransform:"uppercase", letterSpacing:"0.5px" }}>Department</p>
                  {DEPARTMENTS.map(d => <button key={d} onClick={() => { setDeptFilter(d); setPage(1); }} style={{ display:"block", width:"100%", textAlign:"left", padding:"6px 10px", borderRadius:"8px", border:"none", background:deptFilter===d?"#f0f7ff":"transparent", color:deptFilter===d?"#0072c6":"#1a3a4a", fontSize:"13px", fontWeight:deptFilter===d?600:400, cursor:"pointer", marginBottom:"2px" }}>{d}</button>)}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:"14px", marginBottom:"20px" }}>
          {[
            { label:"Total Agents",    value: data.length, color:"#0072c6", bg:"#f0f7ff", Icon: Briefcase },
            { label:"Active",          value: activeCount, color:"#16a34a", bg:"#f0fdf4", Icon: CheckCircle },
            { label:"On Leave",        value: leaveCount,  color:"#d97706", bg:"#fffbeb", Icon: Clock },
            { label:"Managers",        value: managerCount,color:"#9333ea", bg:"#fdf4ff", Icon: Shield },
          ].map(s => (
        ...
        <div style={{ background:"#fff", borderRadius:"16px", boxShadow:"0 2px 16px rgba(0,114,198,0.08)", overflow:"hidden" }}>
          <div style={{ display:"flex", justifyContent:"flex-end", padding:"14px 20px 0" }}>
            <span style={{ fontSize:"12px", color:"#9ab0be" }}>{filtered.length.toLocaleString()} agents</span>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"2fr 1.4fr 100px 90px 80px 40px", padding:"10px 20px", borderBottom:"1px solid #f0f7ff" }}>
            {["AGENT NAME","DEPARTMENT","ROLE","STATUS","RATING",""].map((h,i) => <span key={i} style={{ fontSize:"11px", fontWeight:700, color:"#9ab0be", letterSpacing:"0.5px" }}>{h}</span>)}
          </div>
          ...
          {paginated.length === 0
            ...
            : paginated.map((row, i) => {
              ...
              return (
                <div key={row.id} className="agent-row" style={{ display:"grid", gridTemplateColumns:"2fr 1.4fr 100px 90px 80px 40px", padding:"13px 20px", borderBottom: i < paginated.length-1 ? "1px solid #f5f9ff" : "none", alignItems:"center", transition:"background 0.15s" }}>
                  {/* Agent Info */}
                  ...
                  {/* Department */}
                  <div>
                    <span style={{ background:dc.bg, color:dc.color, fontSize:"11px", fontWeight:600, padding:"3px 10px", borderRadius:"20px", whiteSpace:"nowrap" }}>{row.department}</span>
                  </div>
                  {/* Role */}
                  <span style={{ fontSize:"12px", color:"#5a7a8a", fontWeight:500, display:"flex", alignItems:"center", gap:"4px" }}>
                    {row.role === "Manager" && <Shield size={12} color="#9333ea" />} {row.role}
                  </span>
                  {/* Status */}
                  <span style={{ display:"flex", alignItems:"center", gap:"5px", fontSize:"12px", fontWeight:500, color: row.status==="Active" ? "#16a34a" : row.status==="On Leave" ? "#d97706" : "#9ca3af" }}>
                    <CheckCircle size={13} color={row.status==="Active" ? "#16a34a" : row.status==="On Leave" ? "#d97706" : "#9ca3af"} /> {row.status}
                  </span>
                  {/* Rating */}
                  <Stars n={row.rating} />
                  {/* Actions */}
