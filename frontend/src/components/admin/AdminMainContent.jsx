...
import {
  Ticket, MessageSquare, Users, TrendingUp, TrendingDown, Minus, ChevronDown, Star, CheckCircle
} from "lucide-react";
import {
  LineChart,
  Line,
...
const AdminMainContent = () => {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      ...
      try {
        const { data } = await axiosClient.get(`/admin/messages-leads-analytics?filter=${analyticsFilter}`);
  ...
  const resolutionRate = totalTickets > 0 ? ((resolvedTicketsCount / totalTickets) * 100).toFixed(1) : 0;
  ...
  const stats = [
    { label: "CSAT Score",        value: `${averageRating}/5`, delta: "Rating", up: true,  Icon: Star },
    { label: "Resolution Rate",   value: `${resolutionRate}%`, delta: "Resolved", up: true,  Icon: CheckCircle },
    { label: "Active Tickets",    value: totalTickets - resolvedTicketsCount, delta: "Open",  up: false, Icon: Ticket },
    ...
    <main data-lenis-prevent style={{ flex: 1, overflowY: "auto", padding: "24px" }}>
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
          <h2 style={{ color: "#fff", fontSize: "18px", fontWeight: 700, margin: 0 }}>Organization Performance</h2>
          <div style={{ position: "relative" }}>
            ...
            >
              {filterOptions.map(opt => <option key={opt.value} value={opt.value} style={{ color: "#000" }}>{opt.label}</option>)}
            </select>
            <ChevronDown size={14} color="#fff" style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", pointerEvents: "none" }} />
          </div>
        </div>

        <div style={{ height: "240px", width: "100%" }}>
          {analyticsLoading ? (
            <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.6)", fontSize: "14px" }}>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={analyticsData}>
                ...
                />
                <Tooltip 
                  contentStyle={{ background: "#fff", borderRadius: "10px", border: "none", boxShadow: "0 5px 15px rgba(0,0,0,0.1)" }}
                  itemStyle={{ fontSize: "12px", fontWeight: 600 }}
                  labelStyle={{ color: "#5a7a8a", marginBottom: "4px", fontSize: "11px" }}
                />
                <Area 
                  type="monotone" 
                  dataKey="messages" 
                  ...
                  dot={{ r: 4, strokeWidth: 2, stroke: "#fff", fill: "#1f88d9" }}
      ...
      >
        {stats.map(({ label, value, delta, up, Icon }) => (
          ...
          >
            ...
            >
              <Icon size={18} color={label === "CSAT Score" ? "#f59e0b" : "#0072c6"} />
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {tickets && tickets.length > 0 ? (
              tickets.slice(0, 5).map(ticket => (
                 <div key={ticket._id} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px", borderRadius: "10px", background: "#f8fafc", border: "1px solid #f1f5f9" }}>
                  <div style={{ width: 36, height: 36, background: "#fff", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", border: "1px solid #e2e8f0" }}>
                    <Users size={16} color="#64748b" />
                  </div>
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: 0, fontSize: "13px", fontWeight: 600, color: "#1a3a4a" }}>{ticket.title}</p>
                    <div style={{ display: "flex", gap: "8px", marginTop: "2px" }}>
                      <span style={{ fontSize: "11px", color: "#5a7a8a" }}>Status: {ticket.status}</span>
                      {ticket.rating && (
                        <div style={{ display: "flex", gap: "2px", alignItems: "center" }}>
                          <Star size={10} className="fill-amber-400 text-amber-400" />
                          <span style={{ fontSize: "11px", color: "#f59e0b", fontWeight: 700 }}>{ticket.rating}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  {ticket.feedback && (
                    <div style={{ maxWidth: "200px", fontSize: "11px", color: "#64748b", fontStyle: "italic" }}>
